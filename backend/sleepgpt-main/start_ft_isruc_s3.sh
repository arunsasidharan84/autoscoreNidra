...
#SBATCH --gpus-per-node=3   #This needs to match Trainer(devices=...)
...
#SBATCH --qos=high
#SBATCH --job-name=ft_sleep_isruc_s3
...
srun python3 main_kfold.py with finetune_ISRUC_S3   ISRUC_S3 \
  num_gpus=3 num_nodes=1 num_workers=12 batch_size=10 model_arch=backbone_large_patch200  lr_mult=20 \
  ...
  lr=1e-3 min_lr=0 random_choose_channels=8 max_epoch=100 lr_policy=cosine  drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  ...
  resume_during_training=3 resume_ckpt_path="" actual_channels='ISRUC_S3' gradient_clip_val=5.0 save_top_k=2