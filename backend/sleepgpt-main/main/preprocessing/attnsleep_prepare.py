...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="/data/sleep-edf-database-expanded-1.0.0/sleep-telemetry",
    ...
    if not os.path.exists(args.output_dir):
        ...
        os.makedirs(args.output_dir)
    Data_dir = ['/data/sleep-edf-database-expanded-1.0.0/sleep-cassette', '/data/sleep-edf-database-expanded-1.0.0/sleep-telemetry']
    # Select channel
    ...
    for data_dir in Data_dir:
        ...
        for i in range(len(psg_fnames)):
            ...
            name = os.path.basename(psg_fnames[i]).split('.')[0]
            print(f'******name: {name}*********')
            if name not in "ST7072J0-PSG":
                continue
            ...
            raw_ch = raw_ch_df.values[select_idx[:int(len(labels)*EPOCH_SEC_SIZE * sampling_rate)]]
            raw_ch = raw_ch.transpose(-1, -2)
            # Verify that we can split into 30-s epochsGG:q

            if raw_ch.shape[1] % (EPOCH_SEC_SIZE * sampling_rate) != 0:
                raise Exception("Something wrong")
            n_epochs = raw_ch.shape[1] / (EPOCH_SEC_SIZE * sampling_rate)

            # Get epochs and their corresponding labels
            x = np.asarray(np.split(raw_ch, n_epochs, axis=1)).astype(np.float32)
