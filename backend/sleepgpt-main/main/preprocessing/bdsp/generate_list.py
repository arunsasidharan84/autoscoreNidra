...
import logging
import h5py
Root_path ='/mnt/myvol/data/MGH_NEW'
...
def main():
    ...
    train_nums = nums_anno[idx[:train_len]]
    val_names = names_anno[idx[train_len:train_len+val_len]]
    val_nums = nums_anno[idx[train_len:train_len+val_len]]
    test_names = names_anno[idx[train_len+val_len:]]
    test_nums = nums_anno[idx[train_len+val_len:]]
    ...
    res['names'] = val_names
    res['nums'] = val_nums
    np.save(os.path.join(output_dir, 'val'), arr=res, allow_pickle=True)

    res = {}
    res['names'] = test_names
    res['nums'] = test_nums
    np.save(os.path.join(output_dir, 'test'), arr=res, allow_pickle=True)

    res = {}
    res['names'] = np.concatenate([train_names, names])
    res['nums'] = np.concatenate([train_nums, nums])
    assert len(res['names']) == len(res['nums'])
    np.save(os.path.join(output_dir, 'pre_train'), arr=res, allow_pickle=True)
