...
def main():
    ...
    parser.add_argument("--data_dir", type=str,
                        default="/Volumes/T7/data/sleep-edf-database-expanded-1.0.0/sleep-telemetry",
    ...
    for i in range(len(psg_fnames)):
        ...
        # if psg_fnames[i]!= '/Volumes/T7/data/sleep-edf-database-expanded-1.0.0/sleep-telemetry/ST7121J0-PSG.edf':
        #     continue
        ...
        for a in range(len(ann_stages)):
            ...
            for signal_epoch in range(duration_epoch):
                signals.append(np.copy(psg_f.get_data(tmin=onset_sec+signal_epoch*30, tmax=onset_sec+(signal_epoch+1)*30)))
        ...
        x = signals.astype(np.float)
        # Get epochs and their corresponding labels
        y = labels.astype(np.int32)
        assert y.shape[0] == x.shape[0], f'y: {y.shape[0]}, x: {x.shape[0]}'
