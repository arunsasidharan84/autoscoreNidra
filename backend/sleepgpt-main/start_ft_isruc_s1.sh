...
#SBATCH --gpus-per-node=6   #This needs to match Trainer(devices=...)
...
#SBATCH --qos=high
#SBATCH --job-name=ft_sleep_isruc_s1
...
srun python3 main_kfold.py with finetune_ISRUC_S1   ISRUC_S1 \
  num_gpus=6 num_nodes=1 num_workers=24 batch_size=10 model_arch=backbone_large_patch200  lr_mult=20 \
  ...
  lr=1.25e-3 min_lr=0 random_choose_channels=8 max_epoch=65 lr_policy=cosine  drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  ...
  layer_decay=0.75 Lambda=1.0 patch_size=200 use_cb=True kfold=10 grad_name='all' \
  mode="finetune_noval_ISRUC_S1" \
  resume_during_training=4 resume_ckpt_path="" actual_channels='ISRUC_S1' save_top_k=10