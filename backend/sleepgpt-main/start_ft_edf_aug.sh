...
srun python3 main_kfold.py with finetune_edf  edf_2013 edf_aug_all_datasets  \
  num_gpus=4 num_nodes=1 num_workers=16 batch_size=12 model_arch=backbone_large_patch200  lr_mult=20 \
  ...
  resume_during_training=0 resume_ckpt_path="" EDF_Mode='2013_All' kfold=20 use_triton=False \
# EDF_Mode={Half_Orig = Half dataset to augment, with original datsets, Half=Half dataset to augment and with the same half dataset to process