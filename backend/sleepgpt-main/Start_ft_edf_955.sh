...
#SBATCH --qos=high
#SBATCH --job-name=ft_sleep_edf_955
...
#srun python3 main_kfold.py with finetune_edf  edf_ft_955 \
...
#  lr=1e-3 min_lr=0 random_choose_channels=8 max_epoch=100 lr_policy=cosine loss_function='l1' drop_path_rate=0.5 warmup_steps=0.1 split_len=1 \
#  load_path=$load_path \
...
#  layer_decay=0.75 Lambda=1.0 patch_size=200 use_cb=True kfold=20 grad_name='partial_12'
...
srun python3 main_kfold.py with finetune_edf  edf_ft_955 \
  ...
  lr=1e-3 min_lr=0 random_choose_channels=8 max_epoch=80 lr_policy=cosine loss_function='l1' drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  use_pooling='swin' use_relative_pos_emb=False  mixup=0 smoothing=0.1 decoder_heads=16  use_global_fft=True use_all_label='all' \
  get_param_method='layer_decay'  local_pooling=False optim="adamw" poly=False weight_decay=0.05 \
  layer_decay=0.75 Lambda=1.0 patch_size=200 use_cb=True kfold=30 grad_name='partial_10' save_top_k=2 \
