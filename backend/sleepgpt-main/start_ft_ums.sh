...
#SBATCH --output=./temp_log2/ums/concat/%j.out
# activate conda env
...
srun python3 main_kfold.py with finetune_ums  ums_bjnz_spo2_ods_F3_new_dataset fusion_8_concat_layes \
  ...
  lr=1.5e-3 min_lr=0 random_choose_channels=8 max_epoch=40 lr_policy=cosine loss_function='l1' drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  ...
  resume_during_training=0 resume_ckpt_path="" kfold=15 use_triton=False \
