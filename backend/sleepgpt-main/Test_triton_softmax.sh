...
srun python3 main_kfold.py with finetune_edf  edf_2018 edf_aug_consecutive_datasets  \
  num_gpus=1 num_nodes=1 num_workers=9 batch_size=20 model_arch=backbone_large_patch200  lr_mult=20 \
  ...
  lr=1e-3 min_lr=0 random_choose_channels=8 max_epoch=1 lr_policy=cosine loss_function='l1' drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  ...
  resume_during_training=0 resume_ckpt_path="" EDF_Mode='2018_consecutive' kfold=1 use_triton=False